...
customer_route_bp = Blueprint('customer_route', __name__, subdomain='<subdomain>')
url_setup = URLSetup()

@customer_route_bp.route('/', subdomain="<subdomain>")
...
@customer_route_bp.route('/', subdomain="<subdomain>", methods=['POST'])
def post_message(subdomain):
    result = url_setup.get_owner_data(subdomain)
    if result:
        answers_with_scores = []
        text = f"{result}"
